...
export class ApiService {
  ...
  async getList<T extends Serializable<T>>(type: { new (): T }, path: string, params: any): Promise<T[]> {
    ...
    return API.get<ListData>(path, { params: params, cancelToken: cancelTokenSource.token }).then((value: AxiosResponse<ListData>) => {
      return value.data.data.map((d: any) => new type().deserialize(d));
    });
  }

  async getPagingData<T extends Serializable<T>>(type: { new (): T }, path: string, params: any): Promise<PagingData<T>> {
    let onSuccess = (value: AxiosResponse<PagingData<T>>) => {
  ...
  async postFile<T extends Serializable<T>>(type: { new (): T }, path: string, formData: any, config?: AxiosRequestConfig): Promise<T> {
    return API.post<T>(path, formData, config).then((value) => {
      return new type().deserialize(value.data);
    });
  }

  async putFile<T extends Serializable<T>>(type: { new (): T }, path: string, formData: any, config?: AxiosRequestConfig): Promise<T> {
